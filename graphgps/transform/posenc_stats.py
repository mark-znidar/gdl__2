...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    for t in pe_types:
        if t not in ['LapPE', 'EquivStableLapPE', 'SignNet', 'RWSE', 'HKdiagSE',
                     'HKfullPE', 'ElstaticSE', 'GraphormerBias', 'LHKS']:
    ...
    # Heat Kernels.
    if 'HKdiagSE' in pe_types or 'HKfullPE' in pe_types or 'LHKS' in pe_types:
        ...
        # Cache eigenpairs for Learnable HKS (times are learned in the encoder).
        if 'LHKS' in pe_types:
            num_eigvec = cfg.posenc_LHKS.num_eigvec
            ev = evals_heat.clone()
            evec = evects_heat.clone()

            # L2-normalize eigenvectors (consistent with get_heat_kernels_diag)
            evec = F.normalize(evec, p=2., dim=0)

            # Remove near-zero eigenvalues (constant component, uninformative)
            mask = ev >= 1e-8
            ev = ev[mask]
            evec = evec[:, mask]

            # Keep the first num_eigvec smallest non-zero eigenvalues
            k = min(num_eigvec, len(ev))
            ev = ev[:k]
            evec = evec[:, :k]

            evec_sq = evec ** 2  # HKS uses phi_i(v)^2

            # Pad when graph has fewer non-zero eigenpairs than requested
            if k < num_eigvec:
                ev = F.pad(ev, (0, num_eigvec - k), value=0.0)
                evec_sq = F.pad(evec_sq, (0, num_eigvec - k), value=0.0)

            # Repeat eigenvalues per node so PyG batching concatenates correctly
