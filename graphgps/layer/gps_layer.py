...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        # Compile the pure-tensor inner blocks (no PyG Batch object involved).
        # dynamic=True avoids recompilation when N_max varies across batches.
        # Guarded by hasattr so the code stays compatible with PyTorch < 2.0.
        if hasattr(torch, 'compile'):
            self._sa_block = torch.compile(self._sa_block, dynamic=True)
            self._ff_block = torch.compile(self._ff_block, dynamic=True)
