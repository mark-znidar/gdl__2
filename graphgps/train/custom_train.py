...
# WandB step axis: batch-level grad logs use cur_epoch * STRIDE + batch_idx;
# epoch summaries use cur_epoch * STRIDE + (STRIDE - 1). STRIDE must exceed
# max train batches per epoch so steps stay monotonic.
WANDB_TRAIN_STEP_STRIDE = 10000


def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation,
                cur_epoch=0, wandb_run=None):
    model.train()
    optimizer.zero_grad()
    ...
    for iter, batch in enumerate(loader):
        ...
        loss.backward()
        if wandb_run is not None and iter % 100 == 0:
            wb_step = cur_epoch * WANDB_TRAIN_STEP_STRIDE + iter
            for name, param in model.named_parameters():
                if 'log_times' in name and param.grad is not None:
                    grad = param.grad.detach().cpu()
                    wandb_run.log({
                        "grad/norm": float(grad.norm()),
                        "grad/max": float(grad.abs().max()),
                        "grad/min": float(grad.abs().min()),
                    }, step=wb_step)
                    break
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    wandb_run = run if cfg.wandb.use else None
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        start_time = time.perf_counter()
        train_epoch(loggers[0], loaders[0], model, optimizer, scheduler,
                    cfg.optim.batch_accumulation, cur_epoch, wandb_run)

        # Extract learnable diffusion times (LHKS only) so they are saved
        # to stats.json and flow through to wandb via flatten_dict(perf).
        diff_stats = {}
        for name, param in model.named_parameters():
            if 'log_times' in name:
                times = torch.exp(param).detach().cpu().numpy()
                diff_stats = {f"diffusion_time/t{j}": round(float(t), cfg.round)
                              for j, t in enumerate(times)}
                diff_stats["diffusion_time/t_min"] = round(float(times.min()), cfg.round)
                diff_stats["diffusion_time/t_max"] = round(float(times.max()), cfg.round)
                diff_stats["diffusion_time/spread"] = round(float(times.max() - times.min()), cfg.round)
                diff_stats["diffusion_time/std"] = round(float(times.std()), cfg.round)
                break

        perf[0].append(loggers[0].write_epoch(cur_epoch,
                                               extra_stats=diff_stats or None))
        ...
        if cfg.wandb.use:
            wb_epoch_step = (cur_epoch * WANDB_TRAIN_STEP_STRIDE
                             + (WANDB_TRAIN_STEP_STRIDE - 1))
            run.log(flatten_dict(perf), step=wb_epoch_step)
        ...
        if is_eval_epoch(cur_epoch):
            ...
            if cfg.metric_best != 'auto':
                ...
                if cfg.wandb.use:
                    ...
                    wb_epoch_step = (cur_epoch * WANDB_TRAIN_STEP_STRIDE
                                     + (WANDB_TRAIN_STEP_STRIDE - 1))
                    run.log(bstats, step=wb_epoch_step)
