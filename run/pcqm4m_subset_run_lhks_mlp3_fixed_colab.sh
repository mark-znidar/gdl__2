#!/usr/bin/env bash
# PCQM4Mv2-Subset L-HKS K=16 MLP layers=3, FIXED diffusion times.
...
#   !bash run/pcqm4m_subset_run_lhks_mlp3_fixed_colab.sh
...
OUTBASE="${REPO_DIR}/results_pcqm4m_subset/mlp_ablation/mlp3_fixed"

echo "=== PCQM4Mv2-Subset L-HKS K=16 MLP3 FIXED-times — Colab (seeds: ${SEEDS[*]}) ==="
echo "Repo:    ${REPO_DIR}"
echo "Config:  ${CFG}"
...
for S in "${SEEDS[@]}"; do
    ...
    mkdir -p "${OUTDIR}"
    echo "============================================================"
    echo ">>> mlp3_fixed seed=${S}  start: $(date)"
    echo ">>> outdir: ${OUTDIR}"
    echo "============================================================"
    python main.py --cfg "${CFG}" --repeat 1 seed "${S}" out_dir "${OUTDIR}"
    echo ">>> mlp3_fixed seed=${S}  done:  $(date)"
    RAN="${RAN} ${S}"
done
