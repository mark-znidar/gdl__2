#!/bin/bash
# Same as run_slurm.sh but only seeds 0 and 1 for RWSE / L-HKS / fixed (10 train jobs).
# K ablations + noPE unchanged (seed 0). OUTBASE defaults to results/slurm1_2seed.
...
OUTBASE="${REPO_DIR}/results/slurm1_2seed"
...
echo "=== Submitting L-HKS experiments (2 seeds: 0,1) ==="
ALL=""

# Exp 0: RWSE baseline
for S in 0 1; do
    J=$(submit "rwse_s${S}" "configs/GPS/slurm2seed/zinc-GPS+RWSE.yaml" $S "${OUTBASE}/exp0_rwse/seed${S}")
    ALL="${ALL}:${J}"; echo "  rwse_s${S} -> ${J}"
done

# Exp 1: L-HKS main
for S in 0 1; do
    J=$(submit "lhks_s${S}" "configs/GPS/slurm2seed/zinc-GPS+LHKS.yaml" $S "${OUTBASE}/exp1_lhks/seed${S}")
    ALL="${ALL}:${J}"; echo "  lhks_s${S} -> ${J}"
done

# Exp 4a: Fixed times
for S in 0 1; do
    J=$(submit "fixed_s${S}" "configs/GPS/slurm2seed/zinc-GPS+LHKS-fixed.yaml" $S "${OUTBASE}/exp4a_fixed/seed${S}")
    ALL="${ALL}:${J}"; echo "  fixed_s${S} -> ${J}"
done

# Exp 4b: K ablations
for K in 4 8 32; do
    J=$(submit "K${K}" "configs/GPS/slurm2seed/zinc-GPS+LHKS-K${K}.yaml" 0 "${OUTBASE}/exp4b_K${K}/seed0")
    ALL="${ALL}:${J}"; echo "  K${K} -> ${J}"
done

# Exp 0b: No PE
J=$(submit "noPE" "configs/GPS/slurm2seed/zinc-GPS-noPE.yaml" 0 "${OUTBASE}/exp0b_noPE/seed0")
ALL="${ALL}:${J}"; echo "  noPE -> ${J}"

DEPS="${ALL#:}"
echo ""
echo "Training jobs (10): $(echo $DEPS | tr ':' ' ')"

# Post-processing (runs after all training)
POST=$(sbatch --parsable \
    --job-name="lhks_post_2seed" \
    --partition="${PARTITION}" ${ACCT_FLAG} \
    --gres="${GRES}" --cpus-per-task="${CPUS}" --mem="${MEM}" \
    --constraint=ampere \
    --time="${TIME_POST}" \
    --dependency="afterok:${DEPS}" \
    --output="${LOGDIR}/post_2seed_%j.out" \
    --error="${LOGDIR}/post_2seed_%j.err" \
    --wrap="
cd ${REPO_DIR}
export XDG_CACHE_HOME=/dfs/user/mznidar/.cache
export TORCH_HOME=\${XDG_CACHE_HOME}/torch
